...
/*******************************************************************************
* Function Name: bob_I2C_I2C_ISR
****************************************************************************//**
*
*  Handles the Interrupt Service Routine for the SCB I2C mode.
*
*******************************************************************************/
CY_ISR(bob_I2C_I2C_ISR)
{
    uint32 diffCount;
    uint32 endTransfer;

#ifdef bob_I2C_I2C_ISR_ENTRY_CALLBACK
    bob_I2C_I2C_ISR_EntryCallback();
#endif /* bob_I2C_I2C_ISR_ENTRY_CALLBACK */

#if (bob_I2C_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
    uint32 response;

    response = bob_I2C_I2C_ACK_ADDR;
    ...
    if(NULL != bob_I2C_customIntrHandler)
    {
        bob_I2C_customIntrHandler();
    }

    if(bob_I2C_CHECK_INTR_I2C_EC_MASKED(bob_I2C_INTR_I2C_EC_WAKE_UP))
    {
    ...
    */
    if(bob_I2C_CHECK_I2C_FSM_MASTER)
    {
        #if(bob_I2C_I2C_MASTER)
        {
            ...
            */
            if(bob_I2C_CHECK_INTR_MASTER_MASKED(bob_I2C_INTR_MASTER_I2C_BUS_ERROR))
            {
                bob_I2C_mstrStatus |= (uint16) (bob_I2C_I2C_MSTAT_ERR_XFER |
                                                         bob_I2C_I2C_MSTAT_ERR_BUS_ERROR);

                endTransfer = bob_I2C_I2C_CMPLT_ANY_TRANSFER;
            }
            ...
            */
            if(bob_I2C_CHECK_INTR_MASTER_MASKED(bob_I2C_INTR_MASTER_I2C_ARB_LOST))
            {
                bob_I2C_mstrStatus |= (uint16) (bob_I2C_I2C_MSTAT_ERR_XFER |
                                                         bob_I2C_I2C_MSTAT_ERR_ARB_LOST);

                endTransfer = bob_I2C_I2C_CMPLT_ANY_TRANSFER;
            }

            #if(bob_I2C_I2C_MULTI_MASTER_SLAVE)
            {
                /* I2C_MASTER_CMD_M_START_ON_IDLE:
                * MultiMaster-Slave does not generate start, because Slave was addressed.
                * Pass control to slave.
                */
                if(bob_I2C_CHECK_I2C_MASTER_CMD(bob_I2C_I2C_MASTER_CMD_M_START_ON_IDLE))
                {
                    bob_I2C_mstrStatus |= (uint16) (bob_I2C_I2C_MSTAT_ERR_XFER |
                                                             bob_I2C_I2C_MSTAT_ERR_ABORT_XFER);
            ...
            */
            if(0u != endTransfer)
            {
                ...
                bob_I2C_mstrStatus |= (uint16) bob_I2C_GET_I2C_MSTAT_CMPLT;
                ...
                #if(bob_I2C_I2C_MULTI_MASTER_SLAVE)
                {
                    if(bob_I2C_CHECK_I2C_FSM_ADDR)
                    {
                        ...
                        */
                        if(bob_I2C_CHECK_I2C_MASTER_CMD(bob_I2C_I2C_MASTER_CMD_M_START_ON_IDLE))
                        {
                            bob_I2C_I2C_MASTER_CLEAR_START;

                            endTransfer = bob_I2C_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        ...
                        */
                        else if((!bob_I2C_CHECK_INTR_SLAVE_MASKED(bob_I2C_INTR_SLAVE_I2C_BUS_ERROR))
                               && bob_I2C_CHECK_INTR_MASTER_MASKED(bob_I2C_INTR_MASTER_I2C_ARB_LOST))
                        {
                            endTransfer = bob_I2C_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        else
                        ...
                        {
                            bob_I2C_CLEAR_TX_FIFO; /* Shifter keeps address, clear it */

                            bob_I2C_DISABLE_MASTER_AUTO_DATA_ACK; /* In case of reading disable autoACK */
                        ...
                        else
                        {
                            ...
                            bob_I2C_state = bob_I2C_I2C_FSM_EXIT_IDLE;
                        }
                    }
                    else
                    {
    ...
    else /* (bob_I2C_CHECK_I2C_FSM_SLAVE) */
    {
        #if(bob_I2C_I2C_SLAVE)
        {
            /* INTR_SLAVE_I2C_BUS_ERROR or bob_I2C_INTR_SLAVE_I2C_ARB_LOST:
            * A Misplaced Start or Stop condition occurred on the bus: set a flag
            * to notify an error condition.
            */
            if(bob_I2C_CHECK_INTR_SLAVE_MASKED(bob_I2C_INTR_SLAVE_I2C_BUS_ERROR |
                                                        bob_I2C_INTR_SLAVE_I2C_ARB_LOST))
            {
                if(bob_I2C_CHECK_I2C_FSM_RD)
                {
                    /* TX direction: master reads from slave */
                    bob_I2C_slStatus &= (uint8) ~bob_I2C_I2C_SSTAT_RD_BUSY;
                    bob_I2C_slStatus |= (uint8) (bob_I2C_I2C_SSTAT_RD_ERR |
                                                          bob_I2C_I2C_SSTAT_RD_CMPLT);
                }
                else
                {
                    /* RX direction: master writes into slave */
                    bob_I2C_slStatus &= (uint8) ~bob_I2C_I2C_SSTAT_WR_BUSY;
                    bob_I2C_slStatus |= (uint8) (bob_I2C_I2C_SSTAT_WR_ERR |
                                                          bob_I2C_I2C_SSTAT_WR_CMPLT);
                }
    ...
    */
    ...
    if(bob_I2C_CHECK_I2C_FSM_MASTER)
    {
        #if(bob_I2C_I2C_MASTER)
        {
            ...
            */
            if(bob_I2C_CHECK_INTR_MASTER_MASKED(bob_I2C_INTR_MASTER_I2C_STOP))
            {
                bob_I2C_ClearMasterInterruptSource(bob_I2C_INTR_MASTER_I2C_STOP);
            ...
            {
                if(bob_I2C_CHECK_I2C_FSM_ADDR) /* Address stage */
                {
                    /* INTR_MASTER_I2C_NACK:
                    * The master sent an address but it was NACKed by the slave. Complete transaction.
                    */
                    if(bob_I2C_CHECK_INTR_MASTER_MASKED(bob_I2C_INTR_MASTER_I2C_NACK))
                    {
                        bob_I2C_ClearMasterInterruptSource(bob_I2C_INTR_MASTER_I2C_NACK);

                        bob_I2C_mstrStatus |= (uint16) (bob_I2C_I2C_MSTAT_ERR_XFER |
                                                                 bob_I2C_I2C_MSTAT_ERR_ADDR_NAK);
                    ...
                    */
                    ...
                    {
                        if(bob_I2C_CHECK_I2C_FSM_RD) /* Reading */
                        {
                        ...
                        else /* Writing */
                        {
                            ...
                            if(0u != bob_I2C_mstrWrBufSize)
                            {
                                /* Enable INTR.TX_EMPTY if there is data to transmit */
                                bob_I2C_SetTxInterruptMode(bob_I2C_INTR_TX_EMPTY);
                            }
                        }
                    }
                }

                if(bob_I2C_CHECK_I2C_FSM_DATA) /* Data phase */
                {
                    if(bob_I2C_CHECK_I2C_FSM_RD) /* Reading */
                    {
                        /* INTR_RX_FULL:
                        ...
                        */
                        if(bob_I2C_CHECK_INTR_RX_MASKED(bob_I2C_INTR_RX_FULL))
                        {
                            /* Calculate difference */
                            diffCount =  bob_I2C_mstrRdBufSize -
                                        (bob_I2C_mstrRdBufIndex + bob_I2C_GET_RX_FIFO_ENTRIES);

                            /* Proceed transaction or end it when RX FIFO becomes FULL again */
                            if(diffCount > bob_I2C_I2C_FIFO_SIZE)
                            {
                                diffCount = bob_I2C_I2C_FIFO_SIZE;
                            }
                            else
                            {
                                if(0u == diffCount)
                                {
                                    bob_I2C_DISABLE_MASTER_AUTO_DATA_ACK;
                            ...
                            {
                                bob_I2C_mstrRdBufPtr[bob_I2C_mstrRdBufIndex] = (uint8)
                                                                                        bob_I2C_RX_FIFO_RD_REG;
                                bob_I2C_mstrRdBufIndex++;
                            }
                        }
                        /* INTR_RX_NOT_EMPTY:
                        ...
                        */
                        else if(bob_I2C_CHECK_INTR_RX_MASKED(bob_I2C_INTR_RX_NOT_EMPTY))
                        {
                            /* Put data in component buffer */
                            bob_I2C_mstrRdBufPtr[bob_I2C_mstrRdBufIndex] = (uint8) bob_I2C_RX_FIFO_RD_REG;
                            bob_I2C_mstrRdBufIndex++;

                            if(bob_I2C_mstrRdBufIndex < bob_I2C_mstrRdBufSize)
                            {
                    ...
                    else /* Writing */
                    {
                        ...
                        */
                        if(bob_I2C_CHECK_INTR_MASTER_MASKED(bob_I2C_INTR_MASTER_I2C_NACK))
                        {
                            ...
                            bob_I2C_mstrWrBufIndexTmp -= (bob_I2C_GET_TX_FIFO_ENTRIES +
                                                                   bob_I2C_GET_TX_FIFO_SR_VALID);

                            /* Update number of transferred bytes */
                            bob_I2C_mstrWrBufIndex = bob_I2C_mstrWrBufIndexTmp;

                            bob_I2C_mstrStatus |= (uint16) (bob_I2C_I2C_MSTAT_ERR_XFER |
                                                                     bob_I2C_I2C_MSTAT_ERR_SHORT_XFER);

                            bob_I2C_CLEAR_TX_FIFO;
                        ...
                        */
                        else if(bob_I2C_CHECK_INTR_TX_MASKED(bob_I2C_INTR_TX_EMPTY))
                        {
                            while(bob_I2C_I2C_FIFO_SIZE != bob_I2C_GET_TX_FIFO_ENTRIES)
                            {
                                ...
                                */
                                if(bob_I2C_mstrWrBufIndexTmp < bob_I2C_mstrWrBufSize)
                                {
                                    ...
                                    */
                                    if(bob_I2C_mstrWrBufIndexTmp == (bob_I2C_mstrWrBufSize - 1u))
                                    {
                                    ...
                                    bob_I2C_TX_FIFO_WR_REG = (uint32) bob_I2C_mstrWrBufPtr[bob_I2C_mstrWrBufIndexTmp];
                                    bob_I2C_mstrWrBufIndexTmp++;
                                }
                                else
                                {
                        ...
                        #else
                            bob_I2C_ClearTxInterruptSource(bob_I2C_INTR_TX_EMPTY);
                        #endif /* (bob_I2C_CY_SCBIP_V0) */
                        }
                        /* INTR_TX_UNDERFLOW:
                        ...
                        */
                        else if(bob_I2C_CHECK_INTR_TX_MASKED(bob_I2C_INTR_TX_UNDERFLOW))
                        {
                            /* Update number of transferred bytes */
                            bob_I2C_mstrWrBufIndex = bob_I2C_mstrWrBufIndexTmp;
                ...
                if(0u != endTransfer) /* Complete transfer */
                {
                    ...
                    if(bob_I2C_CHECK_I2C_MODE_NO_STOP(bob_I2C_mstrControl))
                    {
                        ...
                        bob_I2C_mstrStatus |= (uint16) (bob_I2C_I2C_MSTAT_XFER_HALT |
                                                                 bob_I2C_GET_I2C_MSTAT_CMPLT);

                        bob_I2C_state = bob_I2C_I2C_FSM_MSTR_HALT;
                    }
                    else
                    {
    ...
    } /* (bob_I2C_CHECK_I2C_FSM_MASTER) */


    /* FSM Slave */
    else if(bob_I2C_CHECK_I2C_FSM_SLAVE)
    {
        #if(bob_I2C_I2C_SLAVE)
        {
            /* INTR_SLAVE_NACK:
            ...
            */
            if(bob_I2C_CHECK_INTR_SLAVE_MASKED(bob_I2C_INTR_SLAVE_I2C_NACK))
            {
                bob_I2C_ClearSlaveInterruptSource(bob_I2C_INTR_SLAVE_I2C_NACK);

                /* All entries that remain in TX FIFO are: FIFO Size + 1 (SHIFTER) */
                diffCount = (bob_I2C_GET_TX_FIFO_ENTRIES + bob_I2C_GET_TX_FIFO_SR_VALID);

                if(bob_I2C_slOverFlowCount > diffCount) /* Overflow */
                {
                    bob_I2C_slStatus |= (uint8) bob_I2C_I2C_SSTAT_RD_OVFL;
                }
                else /* No Overflow */
                {
                    /* Roll-back temporary index */
                    bob_I2C_slRdBufIndexTmp -= (diffCount - bob_I2C_slOverFlowCount);
                }

                /* Update slave of transferred bytes */
                bob_I2C_slRdBufIndex = bob_I2C_slRdBufIndexTmp;
                ...
                bob_I2C_slStatus &= (uint8) ~bob_I2C_I2C_SSTAT_RD_BUSY;
                bob_I2C_slStatus |= (uint8)  bob_I2C_I2C_SSTAT_RD_CMPLT;
                bob_I2C_state     =  bob_I2C_I2C_FSM_IDLE;
                
            #ifdef bob_I2C_I2C_SLAVE_CMPLT_CALLBACK
                /* Read complete */
                bob_I2C_I2C_SlaveCompleteCallback();
            #endif /* bob_I2C_I2C_SLAVE_CMPLT_CALLBACK */
            }
            ...
            */
            if(bob_I2C_CHECK_INTR_SLAVE_MASKED(bob_I2C_INTR_SLAVE_I2C_WRITE_STOP))
            {
                bob_I2C_ClearSlaveInterruptSource(bob_I2C_INTR_SLAVE_I2C_WRITE_STOP);
                ...
                * I2C_ADDR_MATCH.
                */
                if (0u != (bob_I2C_I2C_CTRL_REG & bob_I2C_I2C_CTRL_S_READY_DATA_ACK))
                {
                    while(0u != bob_I2C_GET_RX_FIFO_ENTRIES)
                    {
                        #if(bob_I2C_CHECK_I2C_ACCEPT_ADDRESS)
                        {
                            if((1u == bob_I2C_GET_RX_FIFO_ENTRIES) &&
                               (bob_I2C_CHECK_INTR_SLAVE_MASKED(bob_I2C_INTR_SLAVE_I2C_ADDR_MATCH)))
                            {
                        ...
                        /* Put data in component buffer */
                        bob_I2C_slWrBufPtr[bob_I2C_slWrBufIndex] = (uint8) bob_I2C_RX_FIFO_RD_REG;
                        bob_I2C_slWrBufIndex++;
                    }

                    bob_I2C_DISABLE_SLAVE_AUTO_DATA;
                }

                if(bob_I2C_CHECK_INTR_RX(bob_I2C_INTR_RX_OVERFLOW))
                {
                    bob_I2C_slStatus |= (uint8) bob_I2C_I2C_SSTAT_WR_OVFL;
                }
                ...
                bob_I2C_slStatus &= (uint8) ~bob_I2C_I2C_SSTAT_WR_BUSY;
                bob_I2C_slStatus |= (uint8)  bob_I2C_I2C_SSTAT_WR_CMPLT;
                bob_I2C_state     =  bob_I2C_I2C_FSM_IDLE;
            ...
            */
        #if (bob_I2C_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
            if (bob_I2C_CHECK_INTR_SLAVE_MASKED(bob_I2C_INTR_SLAVE_I2C_ADDR_MATCH |
        ...
        #endif /* (bob_I2C_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
            {
                ...
                bob_I2C_ClearI2CExtClkInterruptSource(bob_I2C_INTR_I2C_EC_WAKE_UP);

                #if (bob_I2C_I2C_CUSTOM_ADDRESS_HANDLER)
                {
                    if (NULL != bob_I2C_customAddressHandler)
                    {
                        /* Call custom address handler */
                        response = bob_I2C_customAddressHandler();
                    }
                    else
                    {
                        /* Read address from the RX FIFO. If there is no address underflow triggers but
                        * component does not use that source. */
                        (void) bob_I2C_RX_FIFO_RD_REG;
                        response = bob_I2C_I2C_ACK_ADDR;
                    }
            ...
            #if (bob_I2C_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
                if (response == bob_I2C_I2C_NAK_ADDR)
                {
                #if (!bob_I2C_CY_SCBIP_V0)
                    /* Disable write stop interrupt source as it triggers after address was NACKed. Ticket ID#156094 */
                    bob_I2C_DISABLE_INTR_SLAVE(bob_I2C_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!bob_I2C_CY_SCBIP_V0) */
                    ...
                    bob_I2C_I2C_SLAVE_GENERATE_NACK;
                }
                else
            #endif /* (bob_I2C_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
                {
                    if(bob_I2C_CHECK_I2C_STATUS(bob_I2C_I2C_STATUS_S_READ))
                    /* TX direction: master reads from slave */
                    {
                        bob_I2C_SetTxInterruptMode(bob_I2C_INTR_TX_EMPTY);

                        /* Set temporary index to address buffer clear from API */
                        bob_I2C_slRdBufIndexTmp = bob_I2C_slRdBufIndex;

                        /* Start master reading */
                        bob_I2C_slStatus |= (uint8) bob_I2C_I2C_SSTAT_RD_BUSY;
                        bob_I2C_state     = bob_I2C_I2C_FSM_SL_RD;
                    }
                    else
                    /* RX direction: master writes into slave */
                    {
                        /* Calculate available buffer size */
                        diffCount = (bob_I2C_slWrBufSize - bob_I2C_slWrBufIndex);
                        ...
                        /* Receive data: into RX FIFO */
                        {
                            if(diffCount == bob_I2C_I2C_FIFO_SIZE)
                            {
                                ...
                                bob_I2C_ENABLE_SLAVE_AUTO_DATA;
                            }
                            else
                            {
                                /* Stretch clock when RX FIFO becomes FULL */
                                bob_I2C_ENABLE_SLAVE_AUTO_DATA_ACK;
                        ...
                        /* Start master reading */
                        bob_I2C_slStatus |= (uint8) bob_I2C_I2C_SSTAT_WR_BUSY;
                        bob_I2C_state     = bob_I2C_I2C_FSM_SL_WR;
                    }
                    ...
                    bob_I2C_ENABLE_INTR_SLAVE(bob_I2C_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!bob_I2C_CY_SCBIP_V0) */

                    /* ACK the address byte */
                    bob_I2C_I2C_SLAVE_GENERATE_ACK;
                }
            }
            ...
            */
            if(bob_I2C_CHECK_INTR_RX_MASKED(bob_I2C_INTR_RX_FULL))
            {
                ...
                diffCount =  bob_I2C_slWrBufSize -
                            (bob_I2C_slWrBufIndex + bob_I2C_I2C_FIFO_SIZE);

                if(diffCount > bob_I2C_I2C_FIFO_SIZE) /* Proceed transaction */
                {
                    diffCount   = bob_I2C_I2C_FIFO_SIZE;
                    endTransfer = 0u;  /* Continue active transfer */
                }
                ...
                {
                    bob_I2C_ENABLE_SLAVE_AUTO_DATA_NACK;
                ...
                bob_I2C_ClearRxInterruptSource(bob_I2C_INTR_RX_FULL);
            }
            ...
            */
            ...
            {
                diffCount = bob_I2C_RX_FIFO_RD_REG;

                if(bob_I2C_slWrBufIndex < bob_I2C_slWrBufSize)
                {
                    bob_I2C_I2C_SLAVE_GENERATE_ACK;

                    /* Put data into component buffer */
                    bob_I2C_slWrBufPtr[bob_I2C_slWrBufIndex] = (uint8) diffCount;
                    bob_I2C_slWrBufIndex++;
                }
                ...
                bob_I2C_ClearRxInterruptSource(bob_I2C_INTR_RX_NOT_EMPTY);
            }
            else
            {
                /* Does nothing */
            ...
            */
            if(bob_I2C_CHECK_INTR_TX_MASKED(bob_I2C_INTR_TX_EMPTY))
            {
                while(bob_I2C_I2C_FIFO_SIZE != bob_I2C_GET_TX_FIFO_ENTRIES)
                {
                    ...
                    */
                    if(bob_I2C_slRdBufIndexTmp < bob_I2C_slRdBufSize)
                    /* Data from buffer */
                    {
                        bob_I2C_TX_FIFO_WR_REG = (uint32) bob_I2C_slRdBufPtr[bob_I2C_slRdBufIndexTmp];
                        bob_I2C_slRdBufIndexTmp++;
                    }
                    else
                    /* Probably Overflow */
                    {
                        bob_I2C_TX_FIFO_WR_REG = bob_I2C_I2C_SLAVE_OVFL_RETURN;

                        if(bob_I2C_slOverFlowCount <= bob_I2C_I2C_TX_OVERFLOW_COUNT)
                        {
                            /* Get counter in range of overflow. */
                            bob_I2C_slOverFlowCount++;
                        }
                    }
                }
...
#ifdef bob_I2C_I2C_ISR_EXIT_CALLBACK
    bob_I2C_I2C_ISR_ExitCallback();
#endif /* bob_I2C_I2C_ISR_EXIT_CALLBACK */
