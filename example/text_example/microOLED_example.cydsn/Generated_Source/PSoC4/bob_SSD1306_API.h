 ...
 * ========================================
*/
#ifndef  __bob_SSD1306_API_H__
    
#define  __bob_SSD1306_API_H__
...
#define Left   0
#define Right  1
int bob_OLED_mode_select(int addr); //Switches display between Command and Display mode
void bob_OLED_Start(); //Initializes the display
void bob_OLED_toggle_Reset(); //Resets display
void bob_OLED_shutdown(); //Puts display in reset
void bob_OLED_wakeup(); //Pulls display out of reset 
...
void bob_OLED_create_default_config(struct bob_OLED_Configuration *defualt_config); //Initializes the display using the defaults neccsary for the Sparkfun microOLED breakout board
void bob_OLED_create_custom_config(struct bob_OLED_Configuration *custom_config,int offset, int start_line, int segment_remap, int scan_direction, int com_pins, int contrast, int mode); //Initializes the display based on a custom configuration

/* Scroll Functions */ 
void bob_OLED_scroll_stop(); //stop scrolling
void bob_OLED_scroll_start(); //start scrolling
void bob_OLED_H_scroll_config(int direction,int start_page,int end_page, int scroll_step); //configure horizontal scrolling 
void bob_OLED_HV_scroll_config(int direction,int start_page,int end_page, int scroll_step, int vertical_offset, int fixed_rows, int scroll_rows); //configure horizontal and vertical scrolling
void bob_OLED_vertical_scroll_area(int fixed_rows, int scroll_rows); //configure vertical scroll area 

/* Addressing Function */ 
void bob_OLED_set_addr_mode(uint8 mode); //Sets addressing mode of the display, currently only supports page addressing mode
void bob_OLED_set_page_addr(uint8 page); //Sets page of display memory to begin writing to
void bob_OLED_set_column_addr(uint8 column); //Sets display column to begin writing to

/*Data Functions*/ 
void bob_OLED_bulk_write(uint8 *data, uint8 x, uint8 y);//Writes data from buffer to display  
void bob_OLED_clear_display(); //Wipes display, sets all pixels value to 0
int bob_OLED_write_characters(char *data, uint8 row, uint8 column, uint8 length); //Writes characters to display

/*Graphics Functions*/ 
uint8 bob_OLED_draw_horizontal_line(uint8 x, uint8 y, uint8 length); //Draw horizontal line
uint8 bob_OLED_draw_vertical_line(uint8 x, uint8 y, uint8 length); //Draw vertical line
uint8 bob_OLED_draw_rectangle(uint8 x, uint8 y, uint8 width, uint8 height, uint8 fill); //Draw rectangle on display 


static uint8_t bob_screenmemory[] = {}; //Buffer to hold data to write to display 
/*Array of pre-defined characters for the OLED_write_characters function*/ 
static uint8_t bob_Oled_characters[] = {
